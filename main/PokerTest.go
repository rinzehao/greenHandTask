...
func main() {
	...
	content, err = file_get_contents("./main/match_result.json") //  "../test/main/match_result.json"
	if err != nil {
	...
	for _, v := range c.Matches {
		winnum := PlayPoker(v.Alice, v.Bob)
		if winnum != v.Result {
			fmt.Println("------------------出错-------------------")
			fmt.Println("Alice的手牌：", v.Alice)
			fmt.Println("Bob的手牌：", v.Bob)
			break
		}
...
func PlayPoker(acard string, bcard string) int {
	var cardOfAlice string = acard //Alice的手牌，以字符串存储
	var cardOfBob string = bcard   //Bob的手牌，以字符串存储
	...
	var winnum int
	if strings.EqualFold(winner, "Alice") {
		winnum = 1
	} else if strings.EqualFold(winner, "Bob") {
		winnum = 2
	}
	return winnum
}
